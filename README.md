...
### 技术特性
- ✅ **SEO优化** - sitemap生成，IndexNow快速索引，robots配置，自定义URL，meta标签
...
在后台"系统配置"中可以添加第三方统计代码（如 Google Analytics、百度统计等），统计代码会自动输出在所有页面底部。

### IndexNow 快速索引

IndexNow 是一个开放协议，可以即时通知搜索引擎（Bing、Yandex 等）您的内容更新，加快索引速度。

**配置步骤**：
1. 在后台"系统配置"中启用 IndexNow
2. 点击"生成"按钮生成 API Key（32位随机字符串）
3. 填写网站域名（不含 http://）
4. 保存配置后，系统会自动在网站根目录创建验证文件

**自动推送**：
- 视频发布时自动推送
- 视频状态变更为发布时自动推送
- sitemap 生成时批量推送（前100个URL）
