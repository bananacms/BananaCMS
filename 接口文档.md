...
本文档详细介绍 BananaCMS 的 RESTful API 接口，适用于开发原生 APP、小程序、第三方客户端等。
...
**说明：** Token 方式无需服务端处理，客户端删除本地 Token 即可。
...
| type | int | 否 | 分类ID |
| order | string | 否 | 排序：time/hits/score/up |
| year | string | 否 | 年份筛选 |
| area | string | 否 | 地区筛选 |
...
```
GET ?action=search&wd=关键词&page=1&limit=20&type=vod
```
...
| page | int | 否 | 页码 |
| limit | int | 否 | 每页数量 |
| type | string | 否 | 搜索类型：vod/actor/art |
...
```
GET ?action=search.suggest&wd=关键
```
...
| page | int | 否 | 页码 |
| limit | int | 否 | 每页数量 |
| type | string | 否 | 类型：video（短视频）/drama（短剧） |
...
1. **跨域问题**：API 已配置 CORS，支持跨域请求
2. **请求频率**：建议控制请求频率，避免被限流
3. **Token 存储**：客户端应安全存储 Token，如 iOS Keychain、Android EncryptedSharedPreferences
4. **图片地址**：返回的图片地址可能是相对路径，需要拼接站点域名
5. **分页参数**：limit 最大值为 50，超过会被限制
6. **缓存策略**：首页、分类等数据建议客户端缓存，减少请求