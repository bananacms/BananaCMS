...
<body class="bg-[#141414] text-white min-h-screen pb-16 md:pb-0">
            ...
            <div class="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
                <div class="flex items-center space-x-4 md:space-x-8">
                    ...
                    <div class="hidden md:flex space-x-6 text-sm">
                        <a href="/" class="<?php echo empty($type) ? 'text-white font-medium' : 'text-gray-300'; ?> hover:text-white">首页</a>
                        ...
                        <?php if (!empty($navTypes)): ?>
                            ...
                            <?php foreach (array_slice($navTypes, 0, 5) as $navType): ?>
                                <a href="<?php echo xpk_page_url('type', ['id' => $navType['type_id'], 'slug' => $navType['type_en']]); ?>" class="<?php echo ($navType['type_id'] == $currentTopId) ? 'text-white font-medium' : 'text-gray-300 hover:text-white'; ?>"><?php echo htmlspecialchars($navType['type_name']); ?></a>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <!-- 移动端菜单 -->
    <div id="mobileMenu" class="fixed inset-0 bg-black/95 z-40 hidden overflow-y-auto">
        <div class="p-6 pt-20 pb-20">
            ...
            <div class="space-y-4">
                ...
                <div class="border-t border-gray-700 pt-4 mt-4">
                    <p class="text-sm text-gray-500 mb-3">分类</p>
                    <?php if (!empty($navTypes)): ?>
                        <?php foreach ($navTypes as $navItem): ?>
                            <a href="<?php echo xpk_page_url('type', ['id' => $navItem['type_id'], 'slug' => $navItem['type_en']]); ?>" class="block text-xl text-gray-400 py-2 hover:text-white"><?php echo htmlspecialchars($navItem['type_name']); ?></a>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    <a href="/type/all" class="block text-xl text-red-500 py-2 mt-2 hover:text-red-400">查看全部分类 →</a>
                </div>
            </div>
        </div>
    </div>
