        ...
        <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="mb-6 pb-6 border-b border-gray-200">
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <a href="<?= htmlspecialchars($flink['link_url']) ?>" target="_blank" rel="noopener nofollow" class="text-gray-500 hover:text-green-500 text-sm"><?= htmlspecialchars($flink['link_name']) ?></a>
                    <?php endforeach; ?>
                    <?php if ($footerLinkCount > $footerLinkMax): ?>
                    <a href="/link" class="text-green-500 hover:text-green-600 text-sm">更多 &raquo;</a>
                    <?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                <div>
                    ...
                    <div class="space-y-2">
                        <a href="/" class="block text-gray-500 hover:text-green-500 text-sm">首页</a>
                        <a href="/hot" class="block text-gray-500 hover:text-green-500 text-sm">热门</a>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-2">
                        <?php if (!empty($navTypes)): ?>
                            <?php foreach (array_slice($navTypes, 0, 4) as $navItem): ?>
                                <a href="<?php echo xpk_page_url('type', ['id' => $navItem['type_id'], 'slug' => $navItem['type_en']]); ?>" class="block text-gray-500 hover:text-green-500 text-sm"><?php echo htmlspecialchars($navItem['type_name']); ?></a>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-2">
                        ...
                        ?>
                        <a href="/page/<?= htmlspecialchars($fpage['page_slug']) ?>" class="block text-gray-500 hover:text-green-500 text-sm"><?= htmlspecialchars($fpage['page_title']) ?></a>
                        <?php 
                            endif;
                        endforeach; 
                        ?>
                        <a href="/link" class="block text-gray-500 hover:text-green-500 text-sm">友情链接</a>
                    </div>
                </div>
                <div>
                    ...
                    <div class="space-y-2">
                        <?php if (isset($user)): ?>
                            <a href="/user/center" class="block text-gray-500 hover:text-green-500 text-sm">个人中心</a>
                        <?php else: ?>
                            <a href="/user/login" class="block text-gray-500 hover:text-green-500 text-sm">登录</a>
                            <a href="/user/register" class="block text-gray-500 hover:text-green-500 text-sm">注册</a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="text-gray-400 text-sm text-center">
                ...
                <p class="mb-2"><a href="https://beian.miit.gov.cn/" target="_blank" rel="nofollow" class="hover:text-green-500"><?= htmlspecialchars($siteConfig['site_icp']) ?></a></p>
                <?php endif; ?>
                Powered by <a href="https://xpornkit.com" class="text-green-500 hover:text-green-600" target="_blank">香蕉CMS</a>
            </div>
        </div>
    </footer>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t z-30">
        <div class="flex justify-around py-2">
            <a href="/" class="flex flex-col items-center py-1 px-3 text-green-500">
