            ...
            <div class="h-16 flex items-center justify-between">
                <div class="flex items-center space-x-4 md:space-x-8">
                    ...
                    <a href="/" class="flex items-center">
                        ...
                        if ($siteLogo): 
                        ?>
                        <img src="<?php echo htmlspecialchars($siteLogo); ?>" alt="<?php echo $siteName ?? SITE_NAME; ?>" class="h-8">
                        <?php else: ?>
                        <span class="text-green-500 text-xl md:text-2xl font-bold"><?php echo $siteName ?? SITE_NAME; ?></span>
                        <?php endif; ?>
                    </a>
                    <div class="hidden md:flex space-x-6 text-sm">
                        <a href="/" class="<?php echo empty($type) ? 'text-green-500 font-medium' : 'hover:text-green-500'; ?>">首页</a>
                        ...
                        <?php if (!empty($navTypes)): ?>
                            ...
                            <?php foreach (array_slice($navTypes, 0, 5) as $navType): ?>
                                <a href="<?php echo xpk_page_url('type', ['id' => $navType['type_id'], 'slug' => $navType['type_en']]); ?>" class="<?php echo ($navType['type_id'] == $currentTopId) ? 'text-green-500 font-medium' : 'hover:text-green-500'; ?>"><?php echo htmlspecialchars($navType['type_name']); ?></a>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
                </div>
