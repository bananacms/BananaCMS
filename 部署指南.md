# 香蕉CMS 部署指南

## 环境要求

- PHP >= 8.0
- MySQL >= 5.7
- Apache / Nginx

## 部署步骤

### 1. 上传文件

将所有文件上传到服务器 Web 目录

### 2. 运行安装向导

> 💡 建议先配置 SSL 证书，再进行安装

访问 `https://你的域名/install.php`，按向导填写：
- 数据库信息
- 管理员账号
- 自定义后台入口路径（安全考虑，避免使用默认的 admin）
- 站点信息

安装完成后：
- 自动创建 `config/install.lock` 锁定安装
- 自动删除敏感文件（install.php、data.sql、*.md 等）

### 3. 配置伪静态

#### Nginx（宝塔面板）

复制以下内容到站点设置 → 伪静态：

```nginx
# Sitemap（支持分片参数）
location = /sitemap.xml {
    rewrite ^ /sitemap.php last;
}

# 禁止访问敏感目录
location ~ ^/(config|core|models|controllers|views|runtime)/ {
    deny all;
}

# 静态资源直接访问
location /static/ {
    try_files $uri =404;
}

location /upload/ {
    try_files $uri =404;
}

# 前台伪静态（仅处理非文件请求）
location / {
    try_files $uri $uri/ /index.php?s=$uri&$args;
}
```

#### Apache

在网站根目录创建 `.htaccess` 文件：

```apache
<IfModule mod_rewrite.c>
    RewriteEngine On
    RewriteBase /

    # Sitemap（支持分片参数）
    RewriteRule ^sitemap\.xml$ sitemap.php [QSA,L]

    # 前台路由
    RewriteCond %{REQUEST_FILENAME} !-f
    RewriteCond %{REQUEST_FILENAME} !-d
    RewriteRule ^(.*)$ index.php?s=$1 [QSA,L]
</IfModule>

# 禁止访问敏感目录
<FilesMatch "^(config|core|models|controllers|views|runtime)">
    Order deny,allow
    Deny from all
</FilesMatch>
```

> 💡 完整配置文件见 `伪静态/` 目录

### 4. 访问测试

- 前台: `https://你的域名/`
- 后台: `https://你的域名/你设置的后台路径.php`
- API: `https://你的域名/api.php?action=vod.list`
- Sitemap: `https://你的域名/sitemap.xml`

## 管理员账号

管理员账号和后台入口路径在安装向导中设置，没有默认值。

- 后台地址: 安装时自定义（如 `/manage.php`、`/backend.php`）
- 账号密码: 安装时自行设置

## SEO 说明

- `robots.txt` - 已配置，屏蔽后台、搜索、用户页面
- `sitemap.xml` - 支持分片，每片最多5000条URL
  - `/sitemap.xml` - 索引文件
  - `/sitemap.xml?type=main` - 首页、分类等
  - `/sitemap.xml?type=vod&page=1` - 视频分片
  - `/sitemap.xml?type=actor&page=1` - 演员分片
  - `/sitemap.xml?type=art&page=1` - 文章分片
- 搜索页、用户页已添加 `noindex` 标签，防止被刷留痕

## 目录结构

```
bananacms/
├── config/          # 配置文件（禁止外部访问）
├── controllers/     # 控制器
├── core/            # 核心类
├── models/          # 模型
├── runtime/         # 运行时目录（缓存、日志）
├── static/          # 静态资源
├── template/        # 前台模板
├── upload/          # 上传文件
├── views/           # 后台视图
├── index.php        # 前台入口
├── api.php          # API入口
└── [自定义].php     # 后台入口（安装时自定义）
```

## 常见问题

### Q: 页面显示空白
A: 检查 PHP 版本是否 >= 8.0，查看 runtime/logs/ 下的错误日志

### Q: 数据库连接失败
A: 重新运行安装向导 `install.php`，检查数据库信息是否正确

### Q: 上传图片失败
A: 检查 upload 目录权限，确保 PHP 有写入权限

### Q: 采集失败
A: 检查服务器是否允许外部请求，或资源站是否可用

## 相关链接

- Telegram 频道: [@BananaCMS](https://t.me/BananaCMS)
- 导航站: [XPornKit成人导航](https://xpornkit.com/zh)
- 问题反馈: [GitHub Issues](https://github.com/bananacms/BananaCMS/issues)