...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="space-x-2">
        <a href="/admin.php/art_type" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">
            分类管理
        </a>
        <a href="/admin.php/art/add" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
            + 添加文章
        </a>
    </div>
</div>
...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <form method="GET" class="flex gap-4 items-end flex-wrap">
        ...
        <a href="/admin.php/art" class="text-gray-500 hover:text-gray-700 py-2">重置</a>
    </form>
</div>
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm space-x-2">
                    <a href="/admin.php/art/edit/<?= $art['art_id'] ?>" class="text-blue-500 hover:underline">编辑</a>
                    <button onclick="deleteItem('/admin.php/art/delete', <?= $art['art_id'] ?>)" class="text-red-500 hover:underline">删除</button>
                </td>
            </tr>
            <?php endforeach; ?>
            <?php endif; ?>
...
<div class="mt-4 flex justify-center">
    <nav class="flex space-x-1">
        <?php if ($page > 1): ?>
        <a href="?page=<?= $page - 1 ?>&type_id=<?= $typeId ?>&keyword=<?= urlencode($keyword) ?>" class="px-3 py-2 border rounded hover:bg-gray-100">上一页</a>
        <?php endif; ?>
        <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
        <a href="?page=<?= $i ?>&type_id=<?= $typeId ?>&keyword=<?= urlencode($keyword) ?>" 
            class="px-3 py-2 border rounded <?= $i == $page ? 'bg-blue-500 text-white' : 'hover:bg-gray-100' ?>"><?= $i ?></a>
        <?php endfor; ?>
        <?php if ($page < $totalPages): ?>
        <a href="?page=<?= $page + 1 ?>&type_id=<?= $typeId ?>&keyword=<?= urlencode($keyword) ?>" class="px-3 py-2 border rounded hover:bg-gray-100">下一页</a>
        <?php endif; ?>
    </nav>
</div>
<?php endif; ?>
