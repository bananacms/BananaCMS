...
<div class="mt-4 flex justify-center">
    <nav class="flex space-x-1">
        <?php if ($page > 1): ?>
        <a href="?page=<?= $page - 1 ?>&type=<?= $typeId ?>&status=<?= $status ?>&keyword=<?= urlencode($keyword) ?>" 
            class="px-3 py-2 border rounded hover:bg-gray-100">上一页</a>
        <?php endif; ?>
        
        <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
        <a href="?page=<?= $i ?>&type=<?= $typeId ?>&status=<?= $status ?>&keyword=<?= urlencode($keyword) ?>" 
            class="px-3 py-2 border rounded <?= $i == $page ? 'bg-blue-500 text-white' : 'hover:bg-gray-100' ?>">
            <?= $i ?>
        </a>
        <?php endfor; ?>
        ...
        <a href="?page=<?= $page + 1 ?>&type=<?= $typeId ?>&status=<?= $status ?>&keyword=<?= urlencode($keyword) ?>" 
            class="px-3 py-2 border rounded hover:bg-gray-100">下一页</a>
        <?php endif; ?>
    </nav>
</div>
<?php endif; ?>
