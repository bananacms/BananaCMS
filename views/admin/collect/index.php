...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex gap-2">
        <a href="/<?= $adminEntry ?>?s=collect/log" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded flex items-center">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path>
            </svg>
            采集日志
        </a>
        <a href="/<?= $adminEntry ?>?s=collect/cron" class="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded flex items-center">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            定时采集
        </a>
        <a href="/<?= $adminEntry ?>?s=collect/add" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded">
            + 添加采集站
        </a>
    </div>
</div>
        ...
        <tbody class="divide-y">
            ...
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-gray-500">
                    暂无采集站，<a href="/<?= $adminEntry ?>?s=collect/add" class="text-blue-500 hover:underline">点击添加</a>
                </td>
            </tr>
            <?php else: ?>
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm space-x-2">
                    <a href="/<?= $adminEntry ?>?s=collect/bind/<?= $c['collect_id'] ?>" class="text-purple-500 hover:underline">绑定</a>
                    <a href="/<?= $adminEntry ?>?s=collect/run/<?= $c['collect_id'] ?>" class="text-green-500 hover:underline">采集</a>
                    <a href="/<?= $adminEntry ?>?s=collect/edit/<?= $c['collect_id'] ?>" class="text-blue-500 hover:underline">编辑</a>
                    ...
                    <button onclick="deleteItem('/<?= $adminEntry ?>?s=collect/delete', <?= $c['collect_id'] ?>)" class="text-red-500 hover:underline">删除</button>
                </td>
            </tr>
            <?php endforeach; ?>
            <?php endif; ?>
